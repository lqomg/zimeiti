...
export const baseURL = process.env.VUE_APP_APIHOST;
...
const errorHandler = (error: any) => {
    ...
    if (message === 'CustomError') {
        ...
        const { status, request, data = {}, config } = response;
        if (status != 403) {
            ElNotification({
                type: 'error',
                title: `请求错误 ${status}: ${errorText}`,
                message: data.message || data.msg || errorText,
            });
        }
        if (status == 403 && config.url == '/login') {
            ElNotification({
                type: 'error',
                title: `登录失败 ${status}`,
                message: data.msg || errorText,
                duration: 3000
            });
        }
    } else if (!response) {
...
const request = axios.create({
    ...
    timeout: 40000 // 请求超时时间,5000(单位毫秒) / 0 不做限制
});
...
request.interceptors.request.use(
    async (config: AxiosRequestConfig & { cType?: boolean }) => {
        ...
        if (!token && config.url != '/login' && config.url != '/indexData') {
            ElNotification({
                type: 'warning',
                title: '提示',
                message: '请先登录',
        ...
        config.headers[settings.ajaxHeadersTokenKey] = 'Bearer ' + await getToken();
